var tag = {
    // configure this
    repository: 'http://unilexicon.com', // vocabulary repository url
    site: 'localhost', // deployment site domain
    // done configuring
    ...
    init: function(){
        $.getScript(tag.repository + "/js/autocomplete.js");
        $.getScript(tag.repository + "/js/jquery.formset.js");
        $.getScript(tag.repository + "/js/vocab_set.js");
        ...
        $("button.discard").live('click', function(event){
            event.preventDefault();
            chrome.tabs.getSelected(null, function(tab){
                window.localStorage.removeItem(tag.id(tab.url));
            });
